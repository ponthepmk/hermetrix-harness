...
| Context profiles | provider/desk aware | detected window + compression | exact 32k/64k/96k/128k/256k/1M envelopes; qualification ใช้ registry เดียวกันครบถึง `ultra-1m` | **Correct**, แต่หลักฐาน recall ยังเป็น bool เดียวทุก tier (O-6) |
...
| **V-9** | corpus P9-B วัดความสูญเสียไม่ได้อีกแล้วหลังแก้ compactor — ทุก fact เข้าถึงได้ 100% | medium | **เปิดใหม่แล้วปิดอีกครั้ง** — มิติ "ระยะห่างของถ้อยคำ" ตายเมื่อเปิด embedder (far/middle กลับมา 100%) ครั้งนี้เพิ่มมิติ **fact ที่ถูกยกเลิก**: ประวัติมีค่าเดิมที่ประกาศยกเลิกแล้ว กับค่าใหม่ที่มาแทน ทั้งสองประโยคพูดเรื่องเดียวกันด้วยคำเกือบเดียวกัน semantic ranking จึงให้คะแนนพอกันและไม่มีทางรู้ว่าอันไหนใหม่กว่า<br>วัดบน corpus ที่ generate จริง: `superseded/far/middle` เก็บค่าที่ยกเลิกไปแล้ว **6/6** และทิ้งค่าปัจจุบัน **0/6** — เป็น**คำตอบผิดอย่างมั่นใจ** ไม่ใช่คำตอบที่หายไป ซึ่งอันตรายกว่า<br>assertion เพิ่ม `absent` ของค่าเดิม และ report แยกนับ `stale_answers` ออกจากความล้มเหลวธรรมดา |
