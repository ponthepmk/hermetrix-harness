...
| **+** | **Voice input** | เล็ก แทรกได้ทุกจุดหลัง spec 1 |
...
### 8.4 เสียง: local เท่านั้น

เจ้าของเลือกแล้วว่าเสียงต้องประมวลผลในเครื่อง `webkitSpeechRecognition` ของ Chrome
ส่งเสียงขึ้น server ของ Google จึง **ใช้ไม่ได้** กับระบบที่ยึด local-first

ทางที่เหลือคือ whisper ในเครื่องผ่าน endpoint ที่ผู้ใช้ตั้งเอง รูปแบบเดียวกับ provider และ
embeddings: Hermetrix เก็บแค่ URL ไม่เก็บโมเดล ไม่มี endpoint = ปุ่มไมค์ไม่ปรากฏ
ไม่ใช่ปรากฏแล้วกดไม่ได้
