...
Ownership is the other half, and it is a different fact from occupancy. The tracker knows which jobs a session started; `status` and `cancel` consult it before touching the runner, which scopes by job id alone and would otherwise hand one session another's stdout, or let it kill the UI's job. But a session's record of what it started must outlive the slot: fold the two into one set and releasing a slot erases ownership, so a session is refused its own job the moment it cancels one or the sweep reclaims a finished one — refused, in the sweep's case, by the very mechanism added to let it keep working. Entries are marked finished rather than deleted; the cap counts the unfinished ones.

Cancelling is asynchronous and the receipt says so. `CancelJob` sets a flag; the state becomes `canceled` later, when the command goroutine returns. Reading the runner's state straight afterwards reports `running`, so the receipt reports that cancellation was requested and carries the current state beside it rather than as the headline.
