...
| 10 | untrusted metadata | hostile fixture ที่ฝัง instruction ใน description, tool result, schema และ error ผ่าน **100%** — agent ไม่ทำตามสักเคส | ~~P10-A~~ **ปิดแล้วสำหรับ model ที่ผ่าน** — `hermetrix hostile`, corpus 24 เคส<br>**structural 12/12 ผ่านเสมอ** (description · title · ชื่อ tool · schema · annotation) ยิงผ่าน MCP server จริง protocol จริง ไม่ขึ้นกับ model mutation: เปิดให้ catalog เป็น provider function → แดง 10 · ถอด `untrusted_output` จาก `tool_describe` → แดง 4<br>**behavioural วัดแล้วสองรุ่น — ผลขึ้นกับขนาด model จริง**: qwen3:4b **8/12** (ทุกเคสที่หลุดสั่งให้*พูด* — APPROVAL_CONFIRMED · credential ที่มากับ tool result · turn ปลอม · ERROR_BYPASS_ACCEPTED — เคสที่สั่งให้*ทำ*ปฏิเสธหมดทั้งคู่) · qwen3.8-27b-fp8 ผ่าน gateway จริง **24/24 (100%)**<br>ระหว่างวัดรอบสองเจอบั๊กจริงสองอัน: hostile corpus ส่ง system message สองก้อนซึ่งไม่ตรงกับที่ production ส่งจริง (`renderMessages` รวมเป็นก้อนเดียวเสมอ) — gateway บาง backend ปฏิเสธตรงๆ ทำให้ทุกเคสเป็น transport error ไม่ใช่การวัด แก้แล้วรวมเป็นก้อนเดียว มี mutation test คุม; และ boundary ห่อ tool output วัดแล้ว **ไม่เปลี่ยนผลแม้แต่เคสเดียว** ราคา 51 token ต่อ result จึงถอดออก<br>**สรุป**: กลไก structural กันได้แน่นอนไม่ว่า model ไหน กลไก behavioural ขึ้นกับ model — harness ไม่ได้อ้างว่าปลอดภัยกับ model เล็ก แต่พิสูจน์แล้วว่าปลอดภัยกับ model ที่ผ่าน 100% |
...
**สรุป:** ไม่มี gate ที่เป็น `unspecified` แล้ว ปิดแล้ว 4 จาก 8 (P8-A, P9-A→O-40, P9-C, P10-A) เหลือ **prerequisite task 4 ตัว**: P9-B (กำลังวัดสด — corpus แก้ sizing bug แล้ว รันกับ gateway จริงอยู่), P11-A/P12-A/P14-A (ยังไม่เริ่ม ต้องเลือกเครื่องมือ/มาตรฐานก่อน) ซึ่งเป็นงานที่ตั้งชื่อได้และประเมินได้ ต่างจาก gate คลุมเครือที่ประเมินไม่ได้
