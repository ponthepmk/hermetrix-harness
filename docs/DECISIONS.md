...
| ADR-4 | Narrow capability waist | `implemented` | **A** | direct primitives 11 ตัว — workspace 4, session/context/Skill 4 และ deferred catalog `tool_search`/`tool_describe`/`tool_call` 3; 1,500-tool scale test ยืนยันว่า catalog ไม่ทำให้ waist โต<br>token accounting serialize payload จริง: 1,959 จาก budget 3,584 ของ compact-32k (`TestRealPayloadFitsEveryProfileDirectToolBudget`) |
